...
│   │   ├── gopair-chat-service/         # 好友管理、一对一私信、好友请求审批
...
| 私聊 | 好友搜索、好友请求与审批、一对一私信，WebSocket 实时推送好友状态与消息 |
...
| AI 助手 | 前端集成 AI Chat 抽屉，支持对话式 AI 辅助，流式输出响应 |
...
├── 私聊
│   ├── 好友搜索 / 添加 / 审批
│   ├── 一对一私信（WebSocket 实时推送）
│   └── 好友状态通知（useFriendNotification）
│
├── 文件共享
│   ├── 文件上传 / 下载 / 预览（MinIO）
│   ├── 图片自动生成缩略图
...
│   Chat · File · Voice · AI · Private Chat 功能组件                           │
└──────────────┬──────────────────────┬───────────────────────┘
...
└───┬──────┬──────┬──────┬───────┬───────────┬────────┬────┘
    ...
    ▼      ▼      ▼      ▼       ▼           ▼        ▼
┌──────┐┌─────┐┌──────┐┌─────┐┌───────┐┌──────────┐┌─────┐
│ user ││room ││ msg  ││chat ││ voice ││websocket ││admin│
...
> 网关默认路由前缀：`/user`、`/room`、`/message`、`/chat`、`/file`、`/voice`、`/api/ws`、`/admin`
...
# 按需逐个启动微服务（建议顺序：gateway → user → room → message → chat → file → voice → websocket → admin）
...
java -jar apps/backend/gopair-chat-service/target/gopair-chat-service-*.jar
...
| 方法 | 路径 | 说明 | 鉴权 |
|------|------|------|------|
| GET | `/chat/friends` | 获取好友列表 | 是 |
| POST | `/chat/friends/request` | 发送好友请求 | 是 |
| GET | `/chat/friends/requests` | 获取好友请求列表（收到的 / 发出的） | 是 |
| POST | `/chat/friends/requests/{requestId}/accept` | 接受好友请求 | 是 |
| POST | `/chat/friends/requests/{requestId}/reject` | 拒绝好友请求 | 是 |
| DELETE | `/chat/friends/{friendId}` | 删除好友 | 是 |
| GET | `/chat/users/search` | 搜索用户 | 是 |

#### 私信

| 方法 | 路径 | 说明 | 鉴权 |
|------|------|------|------|
| POST | `/chat/messages/private` | 发送私信 | 是 |
| GET | `/chat/messages/private/conversations` | 获取会话列表 | 是 |
| GET | `/chat/messages/private/{friendId}` | 获取与某好友的私信历史 | 是 |
...
| CHAT | `chat` | 聊天消息 |
| FRIEND_REQUEST | `friend_request` | 好友请求通知 |
| FRIEND_STATUS | `friend_status` | 好友状态变更（在线/离线） |
| PRIVATE_MESSAGE | `private_message` | 私信消息 |
...
│   └── useFriendNotification.ts  # 好友状态通知
├── stores/
...
`perf/k6/` 目录提供 K6 压测方案，涵盖容量验证、登录脉冲、消息可靠性三个核心场景，配套 InfluxDB + Grafana 可视化。
...
| 场景2 | `scenarios/auth_pulse.js` | 登录脉冲：M 个并发 VU 同时登录，测量最大并发承受量及 RT 表现 |
| 场景3 | `scenarios/message_basic.js` | 消息发送：单用户高频 / 群聊并发 / 可靠性验证（零丢失），支持 high_freq / group_chat / reliability 三模式 |
...
# 运行压测并输出到 InfluxDB
k6 run --out influxdb=http://localhost:8086 scenarios/message_basic.js
...
│   └── message_basic.js         # 场景3：消息发送可靠性验证
├── scripts/
│   ├── analyze.js              # 压测后日志分析工具
│   └── bootstrap_users_and_tokens.js  # 用户数据生成脚本
